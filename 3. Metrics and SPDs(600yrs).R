...
print("\nCombining all data...")
final_data_2 <- bind_rows(normalized_data)

# Final verification steps
print("\nFinal Data Summary:")
print("Total number of rows:")
print(nrow(final_data_2))

print("\nSample_Seed distribution:")
print(table(final_data_2$Sample_Seed))

print("\nCombinations of Sample_Seed and Sampling_Pct:")
print(table(final_data_2$Sample_Seed, final_data_2$Sampling_Pct))

print("\nUnique combinations of Settlement_Dist and BP_Dist:")
print(table(final_data_2$Settlement_Dist, final_data_2$BP_Dist))

# Save the combined data
print("\nSaving combined data...")
write.csv(final_data_2, "combined_normalized_samples_1to50_600yrs.csv", row.names = FALSE)
print("Data saved successfully!")


##In case when the final data is already saved, load it in
final_data_2<-read.csv("")
...
## in this case rolling mean by 25 years

calculate_metrics_by_settlement_rm <- function(data, k = 25, keep_area = TRUE) {
...
sampling_pcts <- unique(final_data_2$Sampling_Pct)
result_list <- list()

for(pct in sampling_pcts) {
  print(paste("\nProcessing sampling percentage:", pct))
  
  pct_data <- final_data_2 %>% filter(Sampling_Pct == pct)
...
# Combine results
metrics_by_settlement_2 <- bind_rows(result_list)

# Save raw results (all KL and RMSE values for each settlement/combination)
write.csv(metrics_by_settlement_2, "raw_metrics_by_settlement_600yrs_RM25.csv", row.names = FALSE)
...
check_spd_sums(final_data_2)

# Check the number of rows for each sample seed
metrics_by_settlement_2 %>%
...
metrics_by_settlement_2 <- metrics_by_settlement_2 %>%
  rename(
...
# Usage:
statistical_results <- perform_statistical_tests(metrics_by_settlement_2)


# Save results
write.csv(statistical_results, "wilcoxon_ttest_results_by_settlement_RM25(600yrs).csv", row.names = FALSE)


########################STEP 2-2: Plots for RMSE by settlement ########################################

metrics_by_settlement_2<-read.csv("")

# Create long format for RMSE metric
metrics_long_2 <- metrics_by_settlement_2 %>%
  pivot_longer(
...
dist_combinations <- metrics_by_settlement_2 %>%
...
for(i in 1:nrow(dist_combinations)) {
  ...
  ggsave(paste0("rmse_norm_boxplots_", 
                settlement_dist, "_", 
                bp_dist, "_",
                samp_pct, "percent_RM25.png"),
         p_RMSE, width = 10, height = 6)
}

# Create summary statistics
summary_stats <- metrics_long_2 %>%
...
write.csv(summary_stats, "metrics_by_settlement_summary_600yrs.csv", row.names = FALSE)


#########################STEP 2-3 (Figure 7a) : Compute RMSE by settlement and Boxplot visualizationwith facet_wrap#########################
metrics_long_2 <- metrics_long_2 %>%
  mutate(
...
# RMSE norm plot
p_RMSE <- metrics_long_2 %>%
  filter(Sampling_Pct == 30, Metric =="RMSE") %>%  # Adjust percentage as needed
  ggplot(aes(x = factor(Settlement), y = Value, fill = Dataset_Type)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
...
# Save plots
ggsave("rmse_boxplots_facet_600yrs_RM25.png", p_RMSE, width = 20, height = 12)


########################STEP 3-1: Combine SPDs by settlement to create an overall SPD, then compute RMSE########################################

#rolling mean, in this case rolling mean by 25 years
calculate_combined_metrics_rm <- function(data, k = 25, keep_area = TRUE) {
  result <- data %>%
...
# Calculate metrics
metrics_combined_2 <- calculate_combined_metrics_rm(final_data_2)

# Save raw results
write.csv(metrics_combined_2, "raw_metrics_combined__settlements_600yrs_overallSPD_RM25.csv", row.names = FALSE)

# Create summary statistics for both KL and RMSE metrics
summary_stats <- metrics_combined_2 %>%
...
# Save summary statistics
write.csv(summary_stats, "summary_metrics_combined_settlements_600yrs_RM25.csv", row.names = FALSE)



########################STEP 3-2: Merge settlement SPDs into an overall SPD and plot RMSE as boxplots########################################
metrics_long_combined_2 <- metrics_combined_2 %>%
  pivot_longer(
    cols = starts_with("RMSE_"),
    names_to = "Dataset_Type",
    names_pattern = "RMSE_(.*)",
...
dist_combinations <- metrics_combined_2 %>%
...
for(i in 1:nrow(dist_combinations)) {
  ...
  # RMSE norm plot
  p_RMSE <- metrics_long_combined_2 %>%
    filter(Settlement_Dist == settlement_dist,
           BP_Dist == bp_dist) %>%
...
######################## STEP 3-2 (Figure 4a): Combine settlement SPDs into overall SPD and draw RMSE boxplots with facet_wrap ########################################
create_faceted_comparison_plots <- function(metrics_long_combined_2) {
  p_RMSE_faceted <- metrics_long_combined_2 %>%
    filter(Metric == "RMSE") %>%
    ggplot(aes(x = factor(Sampling_Pct), y = Value, fill = Dataset_Type)) +
    geom_boxplot(position = position_dodge(width = 0.8)) +
  ...
  ggsave("RMSE_norm_faceted_all_600.png", 
         p_RMSE_faceted, 
         width = plot_width, 
         height = plot_height)
...
faceted_plots <- create_faceted_comparison_plots(metrics_long_combined_2)


######################## STEP 3-3 (Figure 1a): Combine settlement SPDs into overall SPD and draw RMSE boxplots (original vs. sample only) #######################################
...
ggsave("RMSE_norm_sampled_all_combos_jitter_600yrs_RM25.png", p_RMSE, width = width, height = height)
...
agg_data_2 <- aggregate_spd_data(final_data_2)
head(agg_data_2)
unique(agg_data_2$Dataset)


##### Plot separately for maximum sampling fractions: 20%, 30%, and 40% #####

create_spd_comparison_plots <- function(agg_data,
                                        smooth_k    = 25,
...
create_spd_comparison_plots(agg_data_2)


create_spd_comparison_plots(agg_data_2, smooth_k = 25,
                            percentages = c(30))             # when testing for specific sampling fraction




# ##### Extract and plot just 4 specific combinations (Figure 2) #####

create_spd_comparison_plots_facets <- function(final_data_2) {
  ...
  specific_combos <- tibble(
    Settlement_Dist = c("power law", "power law", "normal", "normal"),
    BP_Dist = c( "normal","skewed","skewed", "normal" ),
    Sample_Seed = c(10, 4, 1, 33),
    Sampling_Pct = 30,
  ...
  plot_data <- plot_data %>%
    ...
    mutate(
      plot_density_smooth =
        zoo::rollmean(plot_density, k = 25, fill = NA, align = "center"),
      plot_density_smooth =
        plot_density_smooth /
  ...
  p <- ggplot() +
    geom_area(data = original_data,
              aes(calBP, plot_density_smooth, fill = Dataset),  # ← 수정
              alpha = 0.8) +
    geom_line(data = other_lines_data,
              aes(calBP, plot_density_smooth, colour = Dataset),  # ← 수정
              linewidth = 3) +
    geom_line(data = weighted_data,
              aes(calBP, plot_density_smooth, colour = Dataset),  # ← 수정
    ...
    theme_bw(base_size = 34) +  # Large base size
    theme(
  ...
  # --- Save plot
  ggsave("SPD_Comparison_Faceted_600yrs.png", p,
...
# Run the function
combined_plot <- create_spd_comparison_plots_facets(agg_data_2)





# STEP 4-2: Merge settlement SPDs into overall SPD and plot SPD for all 50 seeds together ######

# Supplementary Figure 1
create_combined_original_vs_sampled_spd_plots <- function(agg_data_2) {
  ...
  # Save combined plot
  ggsave("Combined_SPD_Plots_OriginalvsSample600yrs.png", p, 
...
combined_plot <- create_combined_original_vs_sampled_spd_plots(agg_data_2)



# For Original vs Resampled30
create_original_vs_resampled_plots <- function(agg_data_2) {
  # Get unique combinations of Settlement_Dist and BP_Dist
  combinations <- agg_data_2 %>%
    select(Settlement_Dist, BP_Dist) %>%
    distinct()
...
create_original_vs_weighted_plots <- function(agg_data_2) {
  # Get unique combinations of Settlement_Dist and BP_Dist
  combinations <- agg_data_2 %>%
    select(Settlement_Dist, BP_Dist) %>%
    distinct()
...
create_original_vs_resampled_plots_wide <- function(agg_data_2) {
  ...
  for(i in 1:nrow(combinations)) {
    ...
    p <- ggplot() +
      ...
      facet_grid(Sampling_Pct ~ ., 
                 labeller = labeller(Sampling_Pct = function(x) paste0(x, "% Sampling"))) +
      scale_x_reverse(limits = c(2300, 1300),
                      breaks = seq(2300, 1300, -200)) +
...
create_original_vs_weighted_plots_wide <- function(agg_data_2) {
  # 50 distinct, vibrant colors
  distinct_colors <- c(
    "#FF4136", "#FF851B", "#FFDC00", "#2ECC40", "#0074D9", "#B10DC9", "#85144b", 
    "#3D9970", "#39CCCC", "#01FF70", "#F012BE", "#7FDBFF", "#870C25", "#FF6B6B", 
...
create_original_vs_sampled_plots(agg_data_2)
create_original_vs_resampled_plots(agg_data_2)   
create_original_vs_weighted_plots(agg_data_2)     

create_original_vs_resampled_plots_wide(agg_data_2)    
create_original_vs_weighted_plots_wide(agg_data_2)    

### Merge all into one
create_combined_comparison_plots_wide <- function(agg_data_2) {
  distinct_colors <- c(
    "#FF4136", "#FF851B", "#FFDC00", "#2ECC40", "#0074D9", "#B10DC9", "#85144b", 
    "#3D9970", "#39CCCC", "#01FF70", "#F012BE", "#7FDBFF", "#870C25", "#FF6B6B", 
    "#4ECDC4", "#45B7D1", "#FDBB5A", "#6A5ACD", "#20B2AA", "#FF69B4", "#8A2BE2", 
...
create_combined_comparison_plots_wide(agg_data_2)
...
create_settlement_plot_from_file_smooth <- function(seed, settlement_dist, bp_dist, file_path = "", smooth_k = 25) {
  ...
  output_filename <- sprintf("Settlement_SPD_S%d_%s_%s(600yrs).png",
                             seed,
                             settlement_dist,
                             bp_dist)
  
  ggsave(output_filename, p, width = 30, height = 16)  # ← 여기 file_path 제거됨
